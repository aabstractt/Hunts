...
public final class FactionDemoteArgument extends Argument {
    ...
    public void execute(@NonNull CommandActor commandActor, @NonNull String s, @NotNull @NonNull String[] args) {
        ...
        factionMember.setRole(FactionRole.MEMBER);
        faction.broadcast(TranslationKey.FACTION_MEMBER_DEMOTED.build(
                factionMember.getName(),
                source.getName(),
                StringUtils.capitalize(FactionRole.MEMBER.name().toLowerCase())
        ));
        ...
        target.sendMessage(TranslationKey.PLAYER_SELF_DEMOTED.build(
                source.getName(),
                StringUtils.capitalize(FactionRole.MEMBER.name().toLowerCase())
        ));
    }
